# Monte Carlo adversarial check of the domination-split float bounds on
# psplib-j301_1 (PSPLIB j30 instance j301_1, converted to IPF; see
# dag_ntwrk_files/psplib-j301_1/RESULTS.md). Same template as
# run_mc_check_water_k32.jl — only the network and durations file change.
# Recomputes the split bounds, persists them to CSV, then samples 50,000
# configurations (25k uniform-interior + 25k random corners) and checks:
# (a) SOUNDNESS - every sampled float lies inside the bounds; (b) ATTAINMENT -
# the bounds are hit by actual configurations (they are, by construction:
# each bound is the float of an explicitly evaluated corner; the MC corners
# provide independent near-attainment evidence). Also reports measured
# microseconds per propagation to substantiate the corner-count cost claim.
# USAGE: julia -t 1 "validation/cpm_v2/run_mc_check_psplib_j301_1.jl"
...
const NET = joinpath(REPO, "dag_ntwrk_files", "psplib-j301_1")
const LOG = open(joinpath(REPO, "validation", "cpm_v2", "mc_check_psplib_j301_1_log.txt"), "w")
logln(args...) = (println(LOG, args...); println(args...))

j = JSON.parsefile(joinpath(NET, "float", "j301_1-cpm-inputs.json"))
ta = j["time_analysis"]
...
edges, outgoing, incoming, source_nodes = read_graph_to_dict(joinpath(NET, "j301_1.EDGES"))
...
varn = sort!([n for n in keys(dI) if dI[n].lo != dI[n].hi])
...
for v in nodes
    ...
    # same snap the fixed interval_analyze_split now applies (DominationSplit.jl) —
    # kept identical here so this independent recomputation isn't itself exposed
    # to the same node-1-style floating-point crossing the module fix addresses.
    abs(fminus) <= 1e-6 && (fminus = 0.0)
    abs(fplus) <= 1e-6 && (fplus = 0.0)
    (fplus < fminus && fminus - fplus < 1e-6) && (fplus = fminus)
    fl[v] = fminus; fu[v] = fplus
end
el = time() - t0
logln(@sprintf("split done: %d runs in %.1fs = %.1f microseconds/run", nruns, el, 1e6*el/nruns))

open(joinpath(REPO, "validation", "cpm_v2", "psplib_j301_1_float_bounds.csv"), "w") do io
