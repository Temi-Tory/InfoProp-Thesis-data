# TIGHTNESS-ENVELOPE sweep on the GRID (framework cvxP). Vary input DISTRIBUTION {triangular, uniform,
# skewed} x WIDTH {0.05,0.10,0.15} x REGIME {perfect nodes, uncertain 0.7}, report soundness + band
# (tightness) vs Monte Carlo. Produces the envelope figure/table: band vs distribution/width/regime.
# Uses the LIVE framework p-box path (the ported cvxP). Output -> data/grid_envelope.csv.
...
open(joinpath(REPO,"InfoPropFrmwrk","Publications","My work","RESS_response","data","grid_envelope.csv"),"w") do io
...
println("# done -> data/grid_envelope.csv")
